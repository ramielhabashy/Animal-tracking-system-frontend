    ...
    <div className="space-y-8">
      <div className={`flex flex-col md:flex-row md:items-end justify-between gap-4 ${isRtl ? 'text-right' : ''}`}>
        <div>
          ...
          <p className="text-[#404943] mt-2 font-medium">{t('tasks.subtitle')}</p>
        </div>
        <div className={`flex items-center gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
          <div className="flex bg-[#F4F4EF] rounded-xl p-1">
            <button
              onClick={() => setViewMode('list')}
              className={`px-3 py-2 rounded-lg text-sm font-semibold transition-all ${viewMode === 'list' ? 'bg-white text-[#002819] shadow-sm' : 'text-[#717973]'}`}
            >
              <MaterialSymbol icon="list" size={18} />
            </button>
            <button
              onClick={() => setViewMode('calendar')}
              className={`px-3 py-2 rounded-lg text-sm font-semibold transition-all ${viewMode === 'calendar' ? 'bg-white text-[#002819] shadow-sm' : 'text-[#717973]'}`}
            >
        ...
        <>
          ...
          {loading ? (
            ...
            <div className="card p-12 text-center">
              ...
              <p className="text-[#717973] text-sm mt-1">{t('tasks.emptyState')}</p>
            </div>
          ) : (
                      ...
                      <div className={`flex items-center gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
                        ...
                        {task.status === 'delivered' && canApprove(task) && (
                          <button onClick={(e) => { e.stopPropagation(); handleApprove(task); }}
                            className="p-3 bg-[#10B981]/15 text-[#059669] hover:bg-[#10B981]/25 rounded-xl transition-colors"
                            title="Approve">
                        ...
                        {task.status === 'delivered' && canReject(task) && (
                          <button onClick={(e) => { e.stopPropagation(); openRejectModal(task); }}
                            className="p-3 bg-[#BA1A1A]/10 text-[#BA1A1A] hover:bg-[#BA1A1A]/20 rounded-xl transition-colors"
                            title="Reject">
                        ...
                        <MaterialSymbol icon={expandedTask === task.id ? 'expand_less' : 'expand_more'} size={24} className="text-[#717973]" />
                      </div>
                    </div>
                  </div>

                  {expandedTask === task.id && (
                    <div className="border-t border-[#F4F4EF] bg-[#F4F4EF]/30 p-6 space-y-6">
      ...
      {showReassignModal && reassignTask && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
            <h3 className="text-2xl font-bold text-[#002819] mb-2">Reassign Task</h3>
            <p className="text-sm text-[#717973] mb-6">{reassignTask.title}</p>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-[#404943] uppercase tracking-wider mb-2">Assign To</label>
          ...
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className={`flex gap-4 pt-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                ...
                <button onClick={handleDeliver} disabled={saving} className="flex-1 bg-[#10B981] text-white py-3 rounded-xl font-bold text-sm hover:bg-[#059669] transition disabled:opacity-50">
